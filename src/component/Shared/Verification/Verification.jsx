...
function Verification() {
	...
	const [registerVerification, { isLoading: isVerifying }] =
		useRegisterVerificationMutation();
	...
	const handleSubmit = async () => {
		try {
			...
			const response = await registerVerification({
				email,
				otp: otpCode,
			}).unwrap();
			...
			// add the access token to localStorage
	...
	return (
		<div className="flex w-full md:flex-row flex-col min-h-screen justify-between items-center lora">
			...
			<div className="md:w-1/2 w-full md:px-40 py-8">
				<img
					src={login_img2}
					className="h-[120px] w-[120px] mb-8 mx-auto"
					alt="Logo"
				/>
				<div className="w-full px-4">
					<div className="text-center space-y-6">
						<h1 className="text-2xl text-[#5B21BD] font-semibold">
							Verify Your Email
						</h1>
						<p className="text-sm text-gray-600">
							We've sent a 4-digit verification code to your email
							address. Please enter it below to reset your
							password.
						</p>
						<h2 className="text-xl font-bold text-[#5B21BD]">
								...
								<input
									...
									className={`w-12 h-12 text-center text-xl border rounded-full pt-2 transition-all focus:outline-none focus:ring-2 ${
										otpError
											...
											: "border-gray-300 focus:ring-[#5B21BD]"
									}`}
								/>
							))}
						</div>
